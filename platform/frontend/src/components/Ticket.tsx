  ...
  return (
    ...
    >
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
        <span className="bf-mono" style={{ fontSize: 12, fontWeight: 500, color: "var(--ink-mute)" }}>
          {code}
        </span>
        {stamp && <Stamp label={stamp.label} tone={stamp.tone} />}
      </header>
      <h3 style={{ margin: "6px 0 0", fontSize: 15, fontWeight: 500, lineHeight: 1.35 }}>{title}</h3>
      {meta && meta.length > 0 && (
        <dl
          style={{
            display: "flex",
        ...
        >
          {meta.map((m) => (
            <div key={m.label} style={{ display: "flex", gap: 6, alignItems: "baseline" }}>
              <dt className="bf-label" style={{ margin: 0 }}>
                {m.label}
              </dt>
              <dd className="bf-mono" style={{ margin: 0, fontSize: 13 }}>
                {m.value}
              </dd>
            </div>
          ))}
