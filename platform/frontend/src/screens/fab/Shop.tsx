...
export default function Shop() {
  ...
  return (
    <div>
      <div role="tablist" aria-label="Shop desk" className="bf-chip-row" style={{ marginBottom: 16 }}>
          ...
          {ndi.data && ndi.data.length > 0 && (
            <LedgerTable<NdiRecord>
              ...
              columns={[
                ...
                { key: "result", label: "Result", render: (n) => <Stamp label={n.result} tone={n.result === "pass" ? "ok" : "bad"} /> },
            ...
            <LedgerTable<OffcutRow>
              ...
              columns={[
                ...
                { key: "heat", label: "Heat", render: (o) => <span className="bf-mono">{o.heat_no ?? "Untraced"}</span> },
                {
                  key: "status",
                  label: "Status",
                  render: (o) => <Stamp label={o.status} tone={o.status === "available" ? "ok" : o.status === "allocated" ? "info" : "mute"} />,
                },
                {
                  key: "act",
                  label: "",
              ...
              {templates.data.map((t) => (
                <li key={t.id} style={{ border: "1px solid var(--rule-strong)", borderRadius: "var(--radius)", padding: "14px 16px", background: "var(--ground-raise)" }}>
                  ...
                  <ol style={{ margin: "8px 0 0", padding: 0, listStyle: "none", display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {t.stages.map((s, i) => (
                      <li key={i}>
                        <Stamp label={s.requires_photo ? `${s.name} · photo` : s.name} tone={s.requires_photo ? "warn" : "mute"} />
                      </li>
                    ))}
                  </ol>
